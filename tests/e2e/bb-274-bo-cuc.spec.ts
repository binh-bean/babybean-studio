...
import { DIEN_THOAI, MAY_TINH, kiemBoCuc as kiemBoCucChung } from "./helpers/kiem-bo-cuc";

const runId = Math.random().toString(36).slice(2, 10);
const NHAN = `Fixture BB-274 ${runId}`;
const sha256 = (s: string) => createHash("sha256").update(s).digest("hex");

export { DIEN_THOAI, MAY_TINH };
...
/**
 * Bốn phép đo (cuộn ngang / chữ tràn / thanh dính đáy che nút / ảnh phủ kín)
 * chuyển hẳn sang `./helpers/kiem-bo-cuc.ts` — dùng chung với BB-275, và
 * quan trọng hơn: đó là một helper THẬT, không phải `*.spec.ts`. BB-275 từng
 * `import` thẳng từ file spec này, khiến Playwright nạp-chạy luôn khối
 * `test.describe` ở dưới như một tác dụng phụ mỗi lần BB-275 chạy (xem ghi
 * chú đầu `kiem-bo-cuc.ts`).
 */
async function kiemBoCuc(page: Parameters<typeof kiemBoCucChung>[0], tenMan: string, kichThuoc: { width: number; height: number }) {
  return kiemBoCucChung(page, kichThuoc, (hau) => tenAnh(`${tenMan}-${hau}`, kichThuoc));
}
