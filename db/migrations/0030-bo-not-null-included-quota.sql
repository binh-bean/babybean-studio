-- ============================================================================
-- Migration: 0030 — Bỏ NOT NULL và DEFAULT của included_quota
--
-- Task BB-117:
-- galleries.included_quota và packages.included_quota trước đây là NOT NULL DEFAULT 20.
-- Con số 20 là mặc định áp đặt (trên thực tế Lark phần lớn gói là 15, hoặc các gói
-- khác, hoặc chưa xác định khi tạo album nháp).
--
-- Theo 0019, hạn mức chuẩn của album được tính qua app.gallery_quota(g.id), ưu tiên
-- bảng dòng hàng gallery_items (tổng số lượng sản phẩm loại edited_photo). Cột
-- galleries.included_quota chỉ đóng vai trò dự phòng (fallback) khi album chưa có
-- dòng hàng nào.
--
-- Khi chưa biết hạn mức, app.gallery_quota() trả về NULL. Giờ đây:
-- 1. Bỏ NOT NULL và DEFAULT của galleries.included_quota và packages.included_quota.
-- 2. Cập nhật RPC get_admin_galleries:
--    - Dùng app.gallery_quota(g.id) làm nguồn hạn mức thay cho cột tĩnh.
--    - Khi included_quota IS NULL:
--      + extraCount là NULL (không phải 0 — 0 là đủ quota, NULL là chưa biết).
--      + progress hiển thị "N/?" (ví dụ "0/?", "3/?").
-- ============================================================================

begin;

-- 1. Bỏ NOT NULL và DEFAULT trên galleries.included_quota
alter table galleries
  alter column included_quota drop not null,
  alter column included_quota drop default;

comment on column galleries.included_quota is
  'Hạn mức ảnh dự phòng (fallback) khi album chưa có dòng hàng gallery_items. Null khi chưa biết hạn mức.';

-- 2. Bỏ NOT NULL và DEFAULT trên packages.included_quota
alter table packages
  alter column included_quota drop not null,
  alter column included_quota drop default;

comment on column packages.included_quota is
  'Số ảnh chỉnh đi kèm gói chụp theo danh mục. Null nếu gói không cố định hạn mức.';

-- 3. Cập nhật RPC get_admin_galleries
