-- 0068 — BB-200 (2/3): get_admin_galleries trả thêm bốn cột Lark cho màn quản trị.
--
-- Vì sao cần: 0067 thêm lark_trang_thai/lark_canh_bao/lark_trang_thai_tu/
-- lark_doc_luc vào bảng galleries, nhưng RPC danh sách dựng jsonb bằng danh
-- sách cột CỐ ĐỊNH — cột mới không tự lọt vào. Màn danh sách /admin/galleries
-- (bảng lẫn thẻ) cần bốn cột này để tính nhãn và mức cảnh báo bằng
-- `nhanHienThi()`/`mauCanhBao()` phía TypeScript (src/lib/lark/trang-thai-hau-ky.ts) —
-- RPC chỉ CHUYỂN TIẾP mã thô, không tính luật ở đây, để luật chỉ nằm một chỗ.
--
-- DỰNG TỪ 0030, KHÔNG PHẢI 0013: 0030 là bản `get_admin_galleries` mới nhất
-- trước migration này (dùng `app.gallery_quota()` làm nguồn hạn mức thay cột
-- tĩnh, và khi `included_quota` null thì `extraCount` là null + `progress`
-- hiện "N/?"). Một bản nháp trước của 0068 lỡ dựng từ 0013 (bản CŨ hơn, không
-- có hai điều đó) rồi áp lên bb-dev, khiến hạn mức chưa biết hiện sai; Opus đã
-- vá lại 0030 trên bb-dev. Bài học: LUÔN đối chiếu bản mới nhất bằng
-- `grep -rl "function get_admin_galleries" db/migrations` trước khi viết một
-- migration sửa hàm này, không giả định file có số nhỏ hơn tệp mình vừa đọc
-- là bản cũ nhất.
--
-- Chỉ thêm cột vào SELECT và jsonb_build_object; không đổi điều kiện lọc/sắp
-- xếp/đếm/hạn mức nào khác của 0030.
--
-- *** TỆP NÀY CHƯA ĐƯỢC ÁP LÊN BB-DEV. Chờ Opus soát rồi mới áp. ***
...
-- (`create or replace function` cấp lại EXECUTE cho PUBLIC theo mặc định —
-- AGENTS.md §5b — nên dòng revoke PHẢI đứng SAU khối create ở trên).
