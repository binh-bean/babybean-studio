  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-background">
      <header className="flex items-center justify-between gap-3 border-b px-4 py-3">
        <div>
          <h2 className="text-base font-bold">Mua thêm sản phẩm</h2>
          <p className="text-xs text-muted-foreground">
            In ảnh, làm album hoặc đóng khung cho những tấm ba mẹ đã chọn.
          </p>
        </div>
        <Button variant="outline" size="sm" onClick={onDong}>
          Đóng
        </Button>
      </header>
          ...
          <button
            ...
            className={[
              ...
              nhom === nhomDangXem
                ...
                : "bg-surface-2 text-muted-foreground hover:bg-surface-2/70",
            ].join(" ")}
          >
            {TEN_NHOM[nhom]}
          </button>
        ...
        {theoNhom.length === 0 ? (
          ...
          <ul className="space-y-2">
            {theoNhom.map((m) => {
              ...
              return (
                <li key={m.productId} className="rounded-xl border p-3">
                              ...
                              return (
                                <button
                                  ...
                                  className={[
                                    ...
                                    dangCo ? "border-emerald-500" : "border-transparent",
                                  ].join(" ")}
                                >
                                  ...
                                  />
                                  {dangCo && (
                                    <span className="absolute right-1 top-1 rounded-full bg-emerald-500 px-1.5 text-[11px] font-bold text-white">
                                      {dangCo.quantity}
                                    </span>
                                  )}
                                </button>
      ...
      <footer className="border-t px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-lg font-bold">{formatCurrencyVND(tongTien)}</p>
          </div>
          <Button onClick={onDong}>Xong</Button>
        </div>
        {daMua.length > 0 && (
