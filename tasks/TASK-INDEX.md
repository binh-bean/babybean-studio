...
| BB-052 | Rà soát: **mọi** hành động ghi dữ liệu đều có `activity_logs` cùng transaction | PM (Claude tự làm) | BB-039 | med | ✅ **DONE 22/09** — quét 34 đường API: **sáu đường ghi dữ liệu mà im lặng**. Đáng kể nhất: sửa dòng hàng hợp đồng (tiền) và mở lại bộ ảnh đã chốt — chính ghi chú đầu tệp `reopen` nói "sáu tháng sau chỉ trả lời được nếu có người viết vào", mà lý do chỉ ghi vào `galleries.reopen_reason`, cột bị lần mở sau ghi đè. Nay có `src/lib/nhat-ky.ts` (không bao giờ ném, hỏng thì kêu) và sáu đường đều ghi. **Tiện tay bịt một lỗ cách ly chi nhánh**: `POST .../sync` chỉ hỏi quyền chứ không hỏi bộ ảnh thuộc chi nhánh nào. Phép thử CẤU TRÚC quét thư mục `src/app/api`: đường mới nào ghi dữ liệu mà không ghi nhật ký thì ĐỎ, trừ khi có dòng lý do trong danh sách ngoại lệ; hai đường ghi nhật ký bên SQL thì phép thử mở thân hàm ra đối chiếu. Kiểm ngược 3 chốt đều ĐỎ |
| BB-053 | E2E E-1…E-12 theo `docs/10-testing-qa.md §5` | PM (Claude tự làm) | BB-165 | high | 🔶 **E-1 XONG 22/09, và nó bắt được một lỗi CHẶN SẢN PHẨM**: `/api/g/submit` đòi `confirmedByName` và `agreed` từ đầu, còn màn khách gửi `{confirmNotes, customerNote}` — sai cả ba trường. Mỗi lần ba mẹ bấm Xác nhận là một cái 400 "Dữ liệu không hợp lệ", và **KHÔNG AI CHỐT ĐƯỢC BỘ ẢNH**. Không phép thử nào đỏ: phép thử đơn vị gọi thẳng API với payload đúng, còn e2e chưa bao giờ đi tới bước chốt. Đã thêm ô tên người xác nhận + ô đồng ý (hai chuỗi i18n nằm sẵn không ai dùng) và sửa tên trường ghi chú. Kiểm ngược: trả payload về như cũ → E-1 ĐỎ. **Còn lại E-2…E-12** (E-4 hết nghĩa vì đã bỏ mã PIN) |
| BB-054 | Rà soát bảo mật: 14 ca ở `docs/05-rbac.md §6` + grep secret trong bundle | PM (Claude tự làm) | — | high | ✅ **DONE 22/09** — đếm thật: **7/14 ca là `it.todo`**, mỗi dòng ghi "Chờ BB-0xx" cho task đã xong hàng tuần trước. Vitest đếm `todo` là "đã lên kế hoạch" chứ không phải "đang hỏng", nên bộ phép thử xanh suốt còn ma trận rỗng một nửa. Bóc ra: 4 ca ĐÃ có phép thử thật ở tệp khác (không ai đối chiếu ra), **2 ca chưa có gì** (vai `viewer` và `suggester` của link khách — nay đo ở CẢ HAI tầng: cửa route ký phiên thật qua cookie, và hàm SQL `patch_selection_batch`), 1 ca hết nghĩa vì mã PIN bỏ ở `0045`. Ma trận nay có cột "phép thử ở đâu", và có phép thử canh chính cái bảng đó: tệp được viện dẫn phải có thật, có đúng ca đó, và `tests/security/` không còn `it.todo` nào. **Bộ phép thử từ 7 todo về 0.** Nửa bí mật trong bundle: `verify:build` 9/9 đạt, quét 109 tệp gửi xuống trình duyệt. Kiểm ngược 3 chốt đều ĐỎ |
...
| BB-061 | Quản lý khách hàng + bé + lịch sử + chống trùng SĐT | PM (Claude tự làm) | BB-024 ✅ | med | ✅ **DONE 22/09** — `/admin/customers`: tra theo tên hoặc SĐT (bỏ dấu cách, ngoặc, +84 rồi mới so, giống cột `phone_normalized`), hồ sơ khách + bé + lịch sử chụp, đánh dấu **trùng SĐT ở chi nhánh khác** (`uq_customers_phone_branch` chỉ chặn trong MỘT chi nhánh). `GET /api/admin/customers`, `GET`/`PATCH /api/admin/customers/:id`, `POST`/`PATCH`/`DELETE` bé. Cảnh báo ngay trên hồ sơ: khách đến từ Lark thì tên, SĐT và ghi chú **sẽ bị lần đồng bộ sau ghi đè**. 12 ca, kiểm ngược 6 chốt: bỏ lọc chi nhánh ở danh sách → ĐỎ, bỏ lọc ở lịch sử chụp → ĐỎ, bỏ `requireBranch` → ĐỎ `200 to be 403`, bỏ che hồ sơ trùng → ĐỎ, `customers:write`→`read` → ĐỎ. Hai chốt hoá ra chỉ làm ĐẸP CÂU BÁO chứ không giữ dữ liệu (khoá ngoại và unique index mới giữ) — đã ghi thẳng vào mã nguồn |
...
| BB-067 | Tải ảnh preview + ZIP ảnh đã chọn + log lượt tải | PM (Claude tự làm) | BB-015 | med | 🔶 **MỘT PHẦN 22/09** — tải ảnh preview đã chạy từ BB-160 (có phép thử e2e). Chặng này làm **xuất danh sách ảnh đã chọn**: quyền `galleries:export` có từ lâu mà **không mở ra cửa nào** — không route, không nút, chỉ có một cụm chữ nằm sẵn trong tệp dịch, và `docs/04` mô tả một đường không tồn tại. Nay `GET /galleries/:id/export` trả `.txt` (danh sách tên file cho Lightroom) hoặc `.csv` (kèm ghi chú từng ảnh và ghi chú chung), chỉ lấy ảnh của lượt chọn CHÍNH, chắn công thức Excel, ghi nhật ký. 6 ca, kiểm ngược 3 chốt. **Còn lại: ZIP ảnh đã chọn** (hiện tải từng tấm theo lô ở máy khách) **và log lượt tải** — log mỗi tấm là ngập nhật ký, nên cần một đường "bắt đầu tải cả bộ" ghi một dòng kèm số lượng |
| BB-068 | Cron: `expire-galleries`, `send-reminders`, `flush-notifications` | PM (Claude tự làm) | BB-039 | med | 🔶 **MỘT PHẦN 22/09** — `expire-galleries` và `flush-notifications` đã chạy từ BB-186/BB-167. Chặng này chữa cái gốc: **488/488 bộ ảnh có `due_at` rỗng** và không đường nào ghi `sent_at`, nên dòng cài đặt "Hạn chốt mặc định" không điều khiển gì và `expire_overdue_galleries()` trả 0 vĩnh viễn. Nay tạo link gửi khách thì đặt `sent_at` (chỉ lần đầu) và `due_at` (theo cài đặt, chỉ khi đang rỗng — tạo lại link KHÔNG gia hạn ngầm); nút Mở lại dời hạn, nếu không lượt cron 18:00 hôm đó tự đóng lại bộ vừa mở; thuật sĩ đọc số ngày từ cài đặt thay vì chôn cứng 7. 6 ca, kiểm ngược 4 chốt đều ĐỎ đúng lý do. **`send-reminders` xong cùng ngày**: lượt cron hằng ngày quét bộ đã gửi đúng mốc trong `gallery.reminder_days`, chưa chốt, rồi bắn thẻ vào nhóm Lark chi nhánh kèm số khách ĐÃ CHE GIỮA và nút mở bộ ảnh. Chống trùng theo `galleryId:ngayThu`, trần 20 tin mỗi lượt, mảng rỗng là tắt hẳn. App KHÔNG nhắn thẳng cho khách (chưa có ZNS/SMS) nên câu chữ trong màn Cài đặt đã sửa thành "Nhắc CSKH gọi khách vào ngày thứ". 7 ca, kiểm ngược 4 chốt |
...
| BB-225 | Thẻ Hành trình bộ ảnh trên màn khách | DEV-FE | | med | DONE |
