...
| BB-108 | **Gỡ công tắc watermark khỏi giao diện.** Nó chạy đủ một vòng nhưng không component nào vẽ watermark — CSKH bật, hệ thống báo đã bật, ảnh vẫn ra sạch. Hứa suông về một thứ bảo vệ không tồn tại | DEV-FE | — | med | ⏳ **ĐÃ GIAO 16/09** — DEV-BE xong phần máy chủ. Brief: `docs/briefs/BB-108-ba-cong-tac-noi-doi.md`. Soát lại 16/09 thấy **ba** công tắc lệch chứ không phải một: thêm `download` ghi cứng `false` (457 bộ trên bb-prod đều `true`) và `requirePin` mặc định `true` (chủ studio chốt **tắt** từ 12/09) |
