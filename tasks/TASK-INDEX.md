...
| BB-177 | **Màn chi tiết phải luôn hiện link app và tình trạng link**, giống khối "Thư mục ảnh gốc". Hiện chỉ có một dòng chữ và một nút bị khoá | DEV-FE | - | low | 🚫 **BỊ CHẶN 17/09** - Không thể lấy link đầy đủ vì DB chỉ lưu hash, đã hiển thị thông tin tình trạng và tiền tố. Ghi chú tại BLOCKERS.md |
| BB-178 | **Địa chỉ màn quản trị đang là UUID.** Chuỗi `f2c8878a-02ba-...` không đọc được. Thay bằng **mã hợp đồng** (`HD_20260824#4884`). **KHÔNG dùng mã khách hàng** - mã đó gồm tên và số điện thoại, đặt vào địa chỉ là lọt ra lịch sử trình duyệt và log máy chủ. Chỉ đổi màn QUẢN TRỊ, tuyệt đối không đổi link khách | DEV-FE | - | med | ✅ **XONG 17/09** |
